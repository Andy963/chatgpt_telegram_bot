    ...
    if audio_file and Path(audio_file).exists():
        try:
            with wave.open(audio_file, "rb") as f:
                ...
                logger.info("audio segments num: %s", num_segments)
                for i in range(num_segments):
                    ...
                    logger.info("reply multi voice done!")
...
async def message_handle(
    ...
    try:
        ...
        answer = await get_answer_from_ai(
            default_model.name, message, context=context_msg
        )
...
async def new_dialog_handle(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        f"{CHAT_MODES[chat_mode]['welcome_message']}", parse_mode=ParseMode.HTML
    )
